...
window.onload = function()
{
	...
	function percePersonaje()
	{
		...
		*/
		...
		if(muere == 0)
		{
			...
			{
				audios("dangerwind.mp3");
				estilo = "viento_hedor";
				txtPercibe = "<b>Hay <b><font color = 'lime'>Hedor </font></b> y <b><font color = 'skyblue'> viento </font></b>está el Wumpus Y un avismo cerca!</b>";
			}
			else
			{
				if(hayAvismo)
				{
					audios("wind.mp3");
					estilo = "viento";
					txtPercibe = "<b><font color = 'skyblue'>Hay viento, existe un avismo cercano</font></b>";
				}
				else
				{
					if(hayWumpus)
					{
						audios("wumpusnear.mp3");
						estilo = "hedor";
						txtPercibe = "<b><font color = 'lime'>Hay Hedor, el Wumpus está cerca<font></b>";
			...
			if(tieneOro)
			{
				...
				txtPercibe += " <b><font color = 'yellow'>GENIAL!TIENES EL ORO!!! <br> <b><font color = 'magenta'> vuelve a tu punto de partida para entregarlo!<font></b>";
				audios("gold.mp3");
			...
			console.log("el personaje esta en: "+ posPersonaje);
			console.log("inicio es:  "+ mundosWumpus[mundo].posAventurero);

			if(llevaOro && posPersonaje[0] == mundosWumpus[mundo].posAventurero[0] && posPersonaje[1] == mundosWumpus[mundo].posAventurero[1] )
			{
				audios("tada.mp3"); // audio para cuando llega a la meta
				setTimeout(youwin, 500); // se hace un retardo de 500 milisegundos y luego se llama a la funcion youwin
				
				function youwin() 
				{
					alert("YOU WIN!!");
					audios("winner.mp3"); // audio de felicitaciones despues de aceptar la caja de alert
				}					
			}
		}
		else
		{
			...
			if(muere == 1)//Avismo..
			{
				audios("hole.mp3");
				estilo = "avismo";
				txtPercibe = " <b><font color = 'black'>El aventurero a muerto, cayó en un avismo...<font></b>";
			}
			else
			{
				audios("eatenbywumpus.mp3");
				estilo = "piso";
				txtPercibe = "<b><font color = 'red'>El aventurero ha muerto!!, el Wumpus lo ha deborado!! <br> <b><font color = 'yellow'>PERDISTE EL ORO si lo tenias contigo!<font></b>";
	...
	{
		var txtPercibe = "<b><font color = 'orange'>NO TIENES FLECHAS DISPONIBLES :( <font></b>";
		var muere = false;
		...
		if(numFlechas > 0)
		{
			...
			audios("arrowgun.mp3");// si hay flechas disponibles suena un disparo
			for(var i = 0; i < mundosWumpus[mundo].posWumpus.length; i++)
			{
				...
				if(muere)
				{
					txtPercibe = "<b><font color = 'pink'> BUEN TIRO! Has ASESINADO al Wumpus!!!<font></b>";
					setTimeout(deadwumpus, 1000);				
					function deadwumpus() 
					{
						wumpusMuere = true;
						audios("deadwumpus.mp3");						
